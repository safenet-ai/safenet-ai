...
class _SecurityRequestsPageState extends State<SecurityRequestsPage> {
  ...
  // Intentionally removed unused requestCard variables

  Widget _requestCard(String requestId, Map<String, dynamic> data) {
    if (data["requestType"]?.toString().toLowerCase() == "panic_alert") {
      return SecurityPanicAlertListItem(
        requestId: requestId,
        data: data,
        onAssign: () => _assignToSelf(requestId),
        onStart: () => _markInProgress(requestId),
        onResolve: () => _showResolveDialog(requestId),
      );
    } else {
      return SecurityRequestListItem(
        requestId: requestId,
        data: data,
        onAssign: () => _assignToSelf(requestId),
        onStart: () => _markInProgress(requestId),
  ...
  // ... continuing with the state class methods (_assignToSelf, etc)
  // (Leaving the rest of the State class alone so we can place the new stateless widgets OUTSIDE the State class)

  // Methods already defined above, but we removed the duplicate _infoRow and _formatShortTime from here
...
class SecurityPanicAlertListItem extends StatelessWidget {
  ...
  const SecurityPanicAlertListItem({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isPending ? Colors.red[50] : Colors.white.withOpacity(0.95),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: Colors.redAccent.withOpacity(isPending ? 1 : 0.5),
            width: isPending ? 2 : 1,
          ),
          boxShadow: [
            BoxShadow(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Icon(Icons.business, size: 16, color: Colors.red.shade800),
  ...
  String _formatShortTime(dynamic timestamp) {
